...
doneLookingForRootNoteOnScreen:
			...
			for (int i = 0; i < clip->noteRows.getNumElements(); i++) {
				...
				if ((yDisplayTo >= 0 && yDisplayTo < displayHeight)
					...
					thisNoteRow->renderRow(
					    ...
					    displayWidth, currentSong->xScroll[NAVIGATION_CLIP], currentSong->xZoom[NAVIGATION_CLIP]);
		...
		void InstrumentClipView::drawMuteSquare(NoteRow * thisNoteRow, uint8_t thisImage[][3],
...
checkIfSelectingRanges:
		...
		static const uint32_t verticalScrollUIModes[] = {UI_MODE_NOTES_PRESSED, UI_MODE_AUDITIONING,
		                                                 UI_MODE_RECORD_COUNT_IN, UI_MODE_DRAGGING_KIT_NOTEROW, 0};
...
shiftAllColour:
		...
		static const uint32_t noteNudgeUIModes[] = {UI_MODE_NOTES_PRESSED, UI_MODE_HOLDING_HORIZONTAL_ENCODER_BUTTON,
		                                            0};
...
wantToEditNoteRowLength:
			...
			// Or, maybe shift everything horizontally
				...
				// Or if no previous Action, go create a new one now.
					...
					action =
					    actionLogger.getNewAction(ACTION_INSTRUMENT_CLIP_HORIZONTAL_SHIFT, ACTION_ADDITION_NOT_ALLOWED);
					if (action) {
			...
			if (!offset || newNumNotes < 1 || newNumNotes > squareWidth) {
				...
				if (offset && lastAction && lastAction->type == ACTION_NOTE_REPEAT_EDIT && lastAction->openForAdditions
					...
					modelStackWithNoteRow->getNoteRow()->editNoteRepeatAcrossAllScreens(
					    squareStart, squareWidth, modelStackWithNoteRow, action, currentClip->getWrapEditLevel(),
					    newNumNotes);
						...
						if (!gotCollision) {
							...
							// Nudge automation at Clip level
							{
								...
								currentClip->paramManager.nudgeAutomationHorizontallyAtPos(
								    editPadPresses[i].intendedPos, offset, lengthBeforeLoop, action,
...
abandonModRegion:
		...
		void InstrumentClipView::fillOffScreenImageStores() {
			...
			if (currentSong->currentClip->output->type == INSTRUMENT_TYPE_KIT) {
				...
				noteRowIndexBottom = getCurrentClip()->noteRows.search(
				    getCurrentClip()->getYNoteFromYDisplay(0, currentSong), GREATER_OR_EQUAL);
			...
			currentSong->currentClip->renderAsSingleRow(modelStack, this, xScroll, xZoom,
			                                            ...
			                                            PadLEDs::occupancyMaskStore[displayHeight + 1], false,
			                                            noteRowIndexTop, 2147483647, 0, displayWidth, true, false);
		...
		void InstrumentClipView::performActualRender(
		    uint32_t whichRows, uint8_t * image, uint8_t occupancyMask[][displayWidth + sideBarWidth], int32_t xScroll,
					...
					if (lastAction && lastAction->type == ACTION_EUCLIDEAN_NUM_EVENTS_EDIT
					...
					else {
						{
							...
							if (action) {
								...
								action->recordNoteArrayChangeDefinitely(clip, modelStack->noteRowId, &noteRow->notes,
								                                        true);
							}
